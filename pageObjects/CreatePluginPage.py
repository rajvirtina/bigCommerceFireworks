...
class CreatePluginPage:
    ...
    def selectApp(self):
        ...
        size = len(apps)
        for i in range(size):
            appText = apps[i].self.driver.find_element(By.XPATH, self.app_link_Xpath).text
            if appText == "Fireworks":
                self.logger.info("**********Fireworks App Available********")
                apps[i].self.driver.find_element(By.XPATH,
                                                 self.btn_img_app_link_Xpath.replace("$Applink", appText)).click()
            else:
                self.logger.info("**********Fireworks App Not Available********")

    def installApp(self):
        self.driver.find_element(By.XPATH, self.btn_install_Xpath).click()
        time.sleep(10)
        self.driver.find_element(By.NAME, self.btn_checkBox_Name).click()
    ...
    def enteringOTP(self, emailHost, mailID, mailPassword, folder):
        ...
        for i in range(4):
            otp_elem = self.driver.find_element(By.XPATH, str([i + 1]))
    ...
    def verifyPlugInCreated(self):
        ...
        if plugIn:
            self.logger.info("**********Fireworks App is Installed Successfully********")
            self.driver.find_element(By.XPATH, self.btn_plugin_Xpath).click()
        else:
            self.logger.info("**********Fireworks App Not Installed********")
    ...
    def uninstallPlugin(self):
        ...
        for i in range(size):
            ...
            if appText == "Fireworks":
                self.logger.info("**********Fireworks App Available under App Section********")
            else:
                self.logger.info("**********Fireworks App Not under App Section********")
